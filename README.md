...
## Инструкция по запуску тестов (ДЗ 4)

В проекте реализовано автоматическое тестирование API с использованием `pytest`, а также нагрузочное тестирование с помощью `Locust`. Тесты покрывают более 90% кода.

### 1. Запуск юнит- и функциональных тестов (Pytest)
Для запуска тестов и генерации отчета о покрытии выполните следующие шаги:

1. Убедитесь, что вы находитесь в корневой папке проекта и у вас активировано виртуальное окружение (`.venv`).
2. Установите зависимости для тестирования (если они еще не установлены):
   ```bash
   pip install pytest pytest-cov pytest-mock httpx locust
   ```
3. Запустите тесты командой:
   ```bash
   python -m pytest tests/ --cov=main --cov-report=html --cov-report=term
   ```
4. В терминале отобразится таблица с результатами тестирования и общим процентом покрытия.
